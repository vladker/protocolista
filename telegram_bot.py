...
def save_result_files(
    audio_path: str, result: dict, tagged: Optional[list] = None, summary: Optional[str] = None
) -> dict:
    ...
    if tagged:
        ...
        logger.info(f"Сохранен тегированный Markdown: {tagged_md_path}")
...
async def send_result(
    ...
    """Отправка результатов пользователю с обработкой ошибок сети"""
            ...
            if len(text) > max_len:
                ...
                try:
                    await context.bot.send_message(
                        chat_id=chat_id, text=f"```txt\n{text}\n```", parse_mode="MarkdownV2"
                    )
                except Exception as e:
                    logger.error(f"Ошибка при отправке транскрипции: {e}")
        except Exception as e:
    ...
    if summary:
        ...
        try:
            await context.bot.send_message(
                chat_id=chat_id, text=f"```md\n# Саммари:\n\n{summary}\n```", parse_mode="MarkdownV2"
            )
        except Exception as e:
            logger.error(f"Ошибка при отправке саммари: {e}")

    # Отправляем файлы
    for file_type, file_path in [
        ("json", "transcription.json"),
        ("tagged", "diarized.json"),
        ("summary", "summary.md"),
    ]:
        if file_type in files:
            logger.info(f"Файл {file_type} отправлен")
            try:
                await context.bot.send_document(
                    chat_id=chat_id, document=open(files[file_type], "rb"), filename=file_path
                )
            except Exception as e:
                logger.error(f"Ошибка при отправке файла {file_type}: {e}")
            finally:
                if file_type in files and os.path.exists(files[file_type]):
                    try:
                        open(files[file_type], "rb").close()
                    except:
                        pass
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent_message = await context.bot.send_message(chat_id=chat_id, text="📥 Получил файл! Начинаю обработку...")
    ...
    try:
        ...
        # Обновляем статус
        await context.bot.edit_message_text(
            chat_id=chat_id, message_id=sent_message.message_id, text="📝 Генерирую саммари (Ollama)... (опционально)"
        )
        ...
        # Обработка сетевых ошибок (httpx.ReadError, httpx.WriteError и т.д.)
        error_msg = f"❌ Сетевая ошибка при обработке файла: {str(e)[:200]}"
        logger.error(f"Сетевая ошибка: {e}")
        await context.bot.edit_message_text(chat_id=chat_id, message_id=sent_message.message_id, text=error_msg)
    except Exception as e:
        ...
        await context.bot.edit_message_text(chat_id=chat_id, message_id=sent_message.message_id, text=error_text)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Здесь можно добавить логирование traceback через logger.exception()
...
async def s2t_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in user_data_store:
        await context.bot.send_message(chat_id=chat_id, text="❌ Сначала отправьте аудиофайл для обработки.")
        return

    user_data = user_data_store[chat_id]
    if "txt" not in user_data["files"]:
        await context.bot.send_message(
            chat_id=chat_id, text="❌ Расшифровка еще не готова. Отправьте аудиофайл для обработки."
        )
        return
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /s2t: {e}")
...
async def s2t_spk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Определяем путь к файлу с тегированной расшифровкой
    tagged_md_path = user_data["files"].get("tagged_md")

    if not tagged_md_path and "tagged_json" in user_data["files"]:
        # Если tagged_md нет, но есть tagged_json, читаем его и конвертируем
        try:
            with open(user_data["files"]["tagged_json"], "r", encoding="utf-8") as f:
                tagged = json.load(f)

            # Создаем Markdown на лету
            md_text = ""
            for seg in tagged:
                speaker = seg.get("speaker", "Unknown")
                text = seg.get("text", "")
                md_text += f"[{speaker}]: {text}\n"

            # Отправляем результат
            max_len = 4000
            if len(md_text) > max_len:
                for i in range(0, len(md_text), max_len):
                    await context.bot.send_message(chat_id=chat_id, text=md_text[i : i + max_len])
            else:
                await context.bot.send_message(chat_id=chat_id, text=f"```txt\n{md_text}\n```", parse_mode="MarkdownV2")
            ...
            await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /s2t_spk: {e}")
            logger.error(f"Ошибка /s2t_spk: {e}")
            return

    if not tagged_md_path:
        await context.bot.send_message(
            chat_id=chat_id, text="❌ Расшифровка со спикерами еще не готова. Отправьте аудиофайл для обработки."
        )
        return

    try:
        with open(tagged_md_path, "r", encoding="utf-8") as f:
            text = f.read()
...
async def md_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data = user_data_store[chat_id]
    md_path = user_data["files"].get("md")

    if not md_path:
        await context.bot.send_message(
            chat_id=chat_id, text="❌ MD файл еще не готов. Отправьте аудиофайл для обработки."
        )
        return
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /md: {e}")
...
async def list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data = user_data_store[chat_id]
    txt_path = user_data["files"].get("txt")
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /list: {e}")
...
async def summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "summary" not in user_data["files"]:
        await context.bot.send_message(
            chat_id=chat_id, text="❌ Сводный протокол еще не готов. Отправьте аудиофайл для обработки."
        )
        return
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /summary: {e}")
...
async def summary_md_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_document(chat_id=chat_id, document=open(summary_path, "rb"), filename="summary.md")
        logger.info(f"Команда /summary_md обработана для chat_id: {chat_id}")

    except Exception as e:
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /summary_md: {e}")
    ...
    try:
        ...
        if "tagged_json" in user_data:
            ...
            if protocol_text:
                ...
                await context.bot.send_message(chat_id=chat_id, text="❌ Не удалось сгенерировать протокол.")
        else:
            await context.bot.send_message(
                chat_id=chat_id, text="❌ Нет данных для генерации протокола. Отправьте аудиофайл для обработки."
            )

    except FileNotFoundError as e:
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Файл не найден: {e}")
        logger.error(f"Файл не найден /protocol: {e}")
    except Exception as e:
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /protocol: {e}")
    ...
    try:
        ...
        if "tagged_json" in user_data:
            ...
            if protocol_text:
                ...
                # Отправляем файл
                await context.bot.send_document(
                    chat_id=chat_id, document=open(protocol_file, "rb"), filename="protocol.md"
                )
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Файл не найден: {e}")
        logger.error(f"Файл не найден /protocol_md: {e}")
    except Exception as e:
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка при обработке команды /protocol_md: {e}")
        logger.error(f"Ошибка /protocol_md: {e}")


# Функция для форматирования текста в Markdown
def format_markdown(text: str) -> str:
    """Форматирует текст в Markdown с правильным экранированием"""
    import re
