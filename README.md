# Аудио транскрибатор с Telegram ботом

Этот проект представляет собой пайплайн для автоматической обработки аудио: транскрипция, диаризация спикеров и генерация саммари. Также входит Telegram бот для удобной обработки файлов.
...
- **Telegram Bot** — удобный интерфейс для обработки файлов
...
```
protocolista/
├── telegram_bot.py    # Telegram бот для обработки файлов
...
После установки зависимостей и настройки токена запустите бота:
...
- **Аудио файлы** - отправьте mp3, wav, m4a, ogg для обработки

### Что делает бот

1. Принимает аудиофайл от пользователя
2. Транскрибирует речь с помощью Whisper
3. Разделяет речь по спикерам с помощью NeMo (опционально)
4. Генерирует саммари с помощью Ollama + Gemma (опционально)
5. Отправляет результат пользователю в виде текста и файлов
...
- Транскрипцию в текстовом виде
- JSON файл с сегментами
- JSON файл с диаризацией (если доступно)
- Саммари в Markdown (если доступно)

### Примечания

- Обработка может занять несколько минут в зависимости от длины аудио
- Для диаризации и саммари необходимы дополнительные ресурсы
- Ollama должна быть запущена для генерации саммари
